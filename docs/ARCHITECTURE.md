...
- `visionai.capabilities.system_info`: the first real, dispatcher-wired capabilities -- `system.time`, `system.date`, `system.battery`, `system.health` -- all read-only, with battery/CPU/memory probes backed by `psutil` and injectable for testing.
...
- `visionai.app`: console entry point (`visionai` script) that dispatches one read-only capability through the full policy + dispatcher path.
