...
- Wired `JsonPermissionStore` into `build_runtime()` through the same shared `policy_context_factory`, so `granted_capabilities` is read fresh on every dispatch instead of being snapshotted or left empty. `PolicyDecision` now has an explicit `requires_permission` flag so the future permission UI can distinguish "ask for permission" from other denials. `.visionai/` is ignored because it is local persistent app state. No registered production capability currently requires a permission grant, and the user-facing permission-grant prompt is still not built.
...
- Phase 2 desktop UI: a minimal main window exists and is tested, now with a Stop control, verified keyboard tab-order/focus behavior, a tray icon, confirmation and permission-grant prompts, read-only diagnostics/settings dialogs, and worker-thread command execution; editable settings, onboarding, and a live screen-reader pass are not built yet.

## Approved Next Tasks

1. Continue Phase 2: tray, diagnostics, read-only settings, confirmation and permission-grant prompts, and non-blocking command execution now exist; onboarding, editable settings, and a better confirmation/permission UX for richer Risk 2+ action summaries are still needed.
...
5. Register a real Risk 2+ production capability so the confirmation and permission-grant prompts are exercised by something other than synthetic test capabilities -- e.g. a "clear audit history" capability (`InMemoryAuditSink`/`JsonlAuditSink` have no delete method yet either, so that would need adding too), which would also finally close Section 15's still-missing "history deletion" mandatory control.
...
- pytest: 192 passed, 93% coverage (headless, via `tests/conftest.py`'s automatic `QT_QPA_PLATFORM=offscreen`)
- Bandit: passed
