...
- Found and fixed a real, previously undiscovered gap while looking for the next well-scoped safety task: `app.py`'s two direct-dispatch calls and `EventOrchestrator`'s default `policy_context_factory` each independently constructed a bare `PolicyContext()`, so `locked_screen` was always `False` and `granted_capabilities` was always empty in the real running app -- the `WindowsLockStateAdapter` and locked-screen policy check were fully implemented and unit-tested, but dead code outside tests, never actually reachable from the CLI or UI. `visionai.runtime.build_runtime()` now builds one shared `policy_context_factory` (exposed on `Runtime`) that both the CLI and the orchestrator use, checking the real lock adapter fresh on every dispatch. Verified live: the real CLI still runs normally against the genuine (unlocked) adapter, and new tests prove a mutating capability is denied end to end through the dispatcher, the orchestrator, and the CLI's argument-parsing path specifically when locked, while a read-only capability still succeeds. Permission grants remain unwired to a persistent store (a separate, larger follow-up -- see Approved Next Tasks and `docs/SECURITY.md` Remaining Risks), so this fix intentionally did not touch that half of the gap.
...
5. Wire `JsonPermissionStore` into `build_runtime()` and build a permission-grant prompt (the same shape as the existing confirmation prompt: a `PolicyDecision`-level flag, a typed request/response, and a `MainWindow` dialog) so a `permission_required` capability -- e.g. a real Risk 2 "clear audit history" capability, which would also finally close Section 15's still-missing "history deletion" mandatory control -- can actually be used, not just correctly denied by default.
...
- pytest: 182 passed, 92% coverage (headless, via `tests/conftest.py`'s automatic `QT_QPA_PLATFORM=offscreen`)
- Bandit: passed
