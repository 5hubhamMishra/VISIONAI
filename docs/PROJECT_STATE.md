...
- Phase 2 desktop UI: a minimal main window exists and is tested, now with a Stop control, verified keyboard tab-order/focus behavior, a tray icon, a confirmation prompt, read-only diagnostics/settings dialogs, and worker-thread command execution; editable settings, onboarding, a live screen-reader pass, and long-running capability handlers that actually poll cancellation tokens are not built yet.

## Approved Next Tasks

1. Continue Phase 2: tray, diagnostics, read-only settings, confirmation prompt, and non-blocking command execution now exist; onboarding, editable settings, a permission-grant prompt, and a better confirmation UX for richer Risk 2+ action summaries are still needed.
...
5. Build a permission-grant prompt (the same shape as the existing confirmation prompt: a `PolicyDecision`-level flag, a typed request/response, and a `MainWindow` dialog) so a `permission_required` capability -- e.g. a real Risk 2 "clear audit history" capability, which would also finally close Section 15's still-missing "history deletion" mandatory control -- can actually be granted by the user, not only by direct store writes.
...
- pytest: 186 passed, 94% coverage (headless, via `tests/conftest.py`'s automatic `QT_QPA_PLATFORM=offscreen`)
- Bandit: passed
