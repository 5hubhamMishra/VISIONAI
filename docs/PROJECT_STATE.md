...
5. Live-review the real `system.clear_history` desktop prompt flow on Windows, including the permission prompt, the follow-up confirmation prompt, and the audit-history refresh after clearing.
...
- pytest: 200 passed, 93% coverage (headless, via `tests/conftest.py`'s automatic `QT_QPA_PLATFORM=offscreen`)
- Bandit: passed
