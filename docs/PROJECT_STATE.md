...
- Phase 2 desktop UI: a minimal main window exists and is tested, now with a Stop control; tray, settings, onboarding, diagnostics, confirmation UI, and a real cancellation-capable (non-blocking) execution model are not built yet.
...
- pytest: 150 passed, 94% coverage (headless, via `tests/conftest.py`'s automatic `QT_QPA_PLATFORM=offscreen`)
- Bandit: passed
