...
visionai --set-api-key
visionai --delete-api-key
```
...
`--ask "<question>"` sends one question to the configured LLM provider and prints its answer. This is conversation only: the reply is printed, never parsed as a command, and never reaches the policy engine or dispatcher -- it has no way to open an app, change a setting, or otherwise act, no matter what it says. By default no provider is configured (`VISIONAI_LLM_PROVIDER` unset, same as `"none"`), and `--ask` prints a fixed message explaining how to enable one rather than silently doing nothing or making a network call. To enable the real Anthropic provider, install the `intelligence` extra (`pip install -e ".[intelligence]"`) and set `VISIONAI_LLM_PROVIDER=anthropic`; `VISIONAI_LLM_MODEL` optionally overrides the default model (`claude-opus-5`). To use a local, fully offline model instead, install the `local_llm` extra (`pip install -e ".[local_llm]"`), set `VISIONAI_LLM_PROVIDER=local`, and set `VISIONAI_LOCAL_MODEL_PATH` to a GGUF model file you already have on disk -- VisionAI never downloads a model on your behalf, so an unset or missing path fails with a clear error instead of silently reaching the network. Each call is a fresh, stateless question -- nothing about it is remembered between calls.

The API key itself can come from either of two places. Setting `VISIONAI_ANTHROPIC_API_KEY=<your key>` works as before and always takes priority. Or, to avoid setting an environment variable in every shell, run `visionai --set-api-key` -- it prompts for the key with hidden input (nothing is echoed, and it's never accepted as a plain command-line argument) and stores it in the real OS credential store (Windows Credential Manager). `visionai --delete-api-key` removes it again. If neither the environment variable nor a stored key is present, `--ask`/`--suggest` show the same explanatory message as always.
...
The Diagnostics button shows a read-only status summary: app/library versions, registered capability count, tray availability, current state, each input subsystem's status (voice input is still disconnected; camera/vision input is available through the Gesture Control button), and the configured LLM provider (`none` by default).
...
Click "Ask AI" to type one question for the configured LLM provider; the answer appears in the result area and nothing else happens -- same as `--ask`, it never dispatches a command or writes an audit entry. Ask AI remembers recent questions and answers from this window session (a small, bounded amount -- older turns are dropped automatically) so a follow-up question can refer back to an earlier one; this history lives only in memory for as long as the window stays open, is never written to disk, and is never sent anywhere except back to the same LLM provider as part of your next question. Click "Clear Conversation" at any time to delete it immediately. Click "Suggest Command" to type a free-text request instead; if the LLM maps it onto one of the same reviewed commands `--suggest` accepts, the result area shows `"Proposed: <what would happen>"` and a real dialog asks `"Execute this command?"` -- only Yes runs it, through the same policy engine and dispatcher as a typed command (so a capability still needing its own permission or confirmation is gated the same way it always is). Suggest Command does not use or add to the Ask AI conversation history. No provider configured shows the same explanatory message `--ask`/`--suggest` show on the console.
