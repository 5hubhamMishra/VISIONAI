...
Click "Ask AI" to type one question for the configured LLM provider; the answer appears in the result area and nothing else happens -- same as `--ask`, it never dispatches a command or writes an audit entry. Click "Suggest Command" to type a free-text request instead; if the LLM maps it onto one of the same reviewed commands `--suggest` accepts, the result area shows `"Proposed: <what would happen>"` and a real dialog asks `"Execute this command?"` -- only Yes runs it, through the same policy engine and dispatcher as a typed command (so a capability still needing its own permission or confirmation is gated the same way it always is). No provider configured shows the same explanatory message `--ask`/`--suggest` show on the console.
